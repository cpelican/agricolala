...
interface ParcelDetailProps {
  parcel: Pick<Parcel, 'id' | 'name' | 'latitude' | 'longitude' | 'width' | 'height' | 'type'>
...
export function ParcelDetail({ parcel, diseases, products, upcomingTreatments, pastTreatments, substanceData }: ParcelDetailProps) {
    ...
    <div className="p-4 space-y-4">
                    ...
                    <div className="mt-2 space-y-2">
                      {treatment.productApplications.map((app, index) => (
                        <div key={index} className="text-sm">
                          <p>{app.product.name} - {app.dose}L/ha</p>
                          <p className="text-muted-foreground">
                            {app.product.composition.map(comp => 
                ...
                {pastTreatments.map((treatment) => (
                  <div key={treatment.id} className="border rounded-lg p-4">
                    <p className="text-sm text-muted-foreground">
                      {treatment.appliedDate && format(treatment.appliedDate, 'PPP', { locale: fr })}
                    </p>
                    <div className="mt-2 space-y-2">
                      {treatment.productApplications.map((app, index) => (
                        <div key={index} className="text-sm">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Track your substance applications across all treatments</CardDescription>
        </CardHeader>
        <CardContent>
