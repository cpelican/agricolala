...
export default async function ParcelPage({ params }: ParcelPageProps) {
  ...
  const substanceData = currentYearTreatments.reduce((acc, treatment) => {
  ...
  const now = new Date();
  const upcomingTreatments = currentYearTreatments.filter(t => 
    !t.appliedDate || new Date(t.appliedDate) > now
  );
  const pastTreatments = currentYearTreatments.filter(t => 
    t.appliedDate && new Date(t.appliedDate) <= now
  );
